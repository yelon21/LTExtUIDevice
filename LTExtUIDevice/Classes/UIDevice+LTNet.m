...
- (NSString *)ltIPaddress{
    ...
    if (!ipAddress||![UIDevice checkIpString:ipAddress]) {
...
//        NSString *ipString = @"0.0.0.0";
        ...
        NSDictionary *dic = [self jsonObjectFromString:ipJson];
//        NSLog(@"dic==%@",dic);
        ...
        if (code == 0) {
            ...
            if (ipString&&[ipString isKindOfClass:[NSString class]]){
                ...
                if ([self checkIpString:ipString]) {
...
+ (BOOL)checkIpString:(NSString *)ipString{
    ...
    BOOL vaild = [predicate evaluateWithObject:self];
    return vaild;
}
